...
├── KIT_GPG.cmd    Launcher GUI (doppio clic per avviare l'interfaccia grafica)
...
Doppio clic su **`KIT_GPG.cmd`** nella root del kit per aprire la GUI.

Richiede PowerShell (già incluso in Windows 10/11). Nessuna installazione aggiuntiva.

La GUI offre:
- Pulsanti per tutte le operazioni (Cifra, Decifra, Verifica, Gestione chiavi, Setup Trust, Diagnostica)
- Drag & drop diretto dei file `.gpg`
- Accesso rapido alle cartelle `out\` e `reports\`
- Log attività e barra di stato

Gli script `.cmd` nella cartella `run\` restano pienamente funzionanti in modo autonomo.
...
| `run\avvia_GUI.cmd` | **Interfaccia grafica WPF** — launcher per tutti gli script |
...
- `avvia_GUI.cmd` + `KIT_GPG_GUI.ps1`: interfaccia grafica WPF portabile, zero installazioni. Avvia PowerShell con `-ExecutionPolicy Bypass` (nessuna modifica al sistema). Supporta drag & drop nativo di file sulla finestra.
...
- `avvia_GUI.cmd` + `KIT_GPG_GUI.ps1`: nuova interfaccia grafica WPF — launcher con 6 pulsanti operazione, drag & drop, visualizzatore report integrato
